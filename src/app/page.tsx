...
import { trackCTAClick, trackPricingPlanView, testGA4Connection, trackEvent } from "@/lib/analytics"
...
export default function HomePage() {
  ...
  // Test GA4 connection on component mount
  React.useEffect(() => {
    // Delay to ensure GA4 is loaded
    setTimeout(() => {
      testGA4Connection();
    }, 1000);
  }, []);

  const handleContactSalesClick = async (location: string, planType?: string) => {
    await trackCTAClick('request_demo', 'Contact Sales', location)
    setShowContactDialog(true)
  }

  const handleStartTrialClick = async (location: string, planType?: string) => {
    await trackCTAClick('start_free_trial', 'Start Free Trial', location)
    setShowContactDialog(true)
  }

  const handleRequestDemoClick = async (location: string, planType?: string) => {
    await trackCTAClick('request_demo', 'Request Demo', location)
    setShowContactDialog(true)
  }
      ...
      <header className="border-b bg-white/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-8">
            <button
              onClick={async () => {
                console.log('🎯 Features nav clicked - starting chatbot_interaction tracking...');

                // Track the chatbot_interaction event
                const trackingSuccess = await trackEvent('chatbot_interaction', {
                ...
                console.log('🎯 chatbot_interaction tracking result:', trackingSuccess ? 'SUCCESS' : 'FAILED');

                // Scroll to features section after tracking
                setTimeout(() => {
          ...
          <Button
            id="btn-contact-sales-header"
            data-ga-id="request_demo"
            onClick={async () => {
              console.log('🎯 Contact Sales (Header) clicked - sending click_request_demo event...');

              // Track the specific event
              const success = await trackEvent('click_request_demo', {
                event_category: 'cta',
                event_label: 'contact_sales_header',
                button_location: 'header',
                button_text: 'Contact Sales'
              });

              console.log('🎯 click_request_demo tracking result:', success ? 'SUCCESS' : 'FAILED');

              // Execute the callback
              handleContactSalesClick('header');
            }}
          >
            Contact Sales
          </Button>
      ...
      <section className="container mx-auto px-4 pt-20 pb-16 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Button
            ...
            size="lg"
            onClick={async () => {
              console.log('🎯 Start Free Trial (Hero) clicked - sending click_start_free_trial event...');

              // Track the specific event
              const success = await trackEvent('click_start_free_trial', {
                event_category: 'cta',
                event_label: 'start_free_trial_hero',
                button_location: 'hero_section',
                button_text: 'Start Free Trial'
              });

              console.log('🎯 click_start_free_trial tracking result:', success ? 'SUCCESS' : 'FAILED');

              // Execute the callback
              handleStartTrialClick('hero');
            }}
        ...
        <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-8">
          <Card className="border-0 shadow-lg">
            ...
            <CardContent className="space-y-4">
              ...
              <Button
                ...
                onClick={async () => {
                  console.log('🎯 Start Free Trial (Pricing) clicked - sending click_start_free_trial event...');

                  // Track the specific event
                  const success = await trackEvent('click_start_free_trial', {
                    event_category: 'cta',
                    event_label: 'start_free_trial_pricing',
                  ...
                  // Execute the callback
                  handlePricingPlanClick('professional');
                }}
              >
                Start Free Trial
              </Button>
          ...
          <Card className="border-2 border-blue-200 shadow-xl relative">
            ...
            <CardContent className="space-y-4">
              ...
              <Button
                id="btn-contact-sales-enterprise"
                data-ga-id="request_demo"
                className="w-full mt-6"
                onClick={async () => {
                  console.log('🎯 Contact Sales (Enterprise) clicked - sending click_request_demo event...');

                  // Track the specific event
                  const success = await trackEvent('click_request_demo', {
                    event_category: 'cta',
                    event_label: 'contact_sales_pricing',
                    button_location: 'pricing_section',
                    button_text: 'Contact Sales',
                  ...
                  // Execute the callback
                  handlePricingPlanClick('enterprise');
                }}
              >
                Contact Sales
              </Button>
      ...
      <section className="bg-slate-900 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          ...
          <Button
            ...
            onClick={async () => {
              console.log('🎯 Get Started Today clicked - sending click_start_free_trial event...');

              // Track the specific event
              const success = await trackEvent('click_start_free_trial', {
                event_category: 'cta',
                event_label: 'get_started_today',
              ...
              // Execute the callback
              handleStartTrialClick('cta_section');
            }}
          >
            Get Started Today
            <ArrowRight className="ml-2 h-5 w-5" />
